...
@Component({
    selector: "transport-data-truck-detail-edit",
    templateUrl: "./transport-data-truck-detail-edit.component.html",
    styleUrls: ["../../styles/style-of-detail-edit.component.scss"],
})
export class TransportDataTruckDetailEditComponent implements OnInit, OnDestroy {
    ...
    getCarDataAndTrailerData(): void {
        ...
        this.carService.getTruckByCompany(this.authService.getUser.CompanyId)
    ...
    refreshTrailerArray(carType: number): void {
        ...
        if (this.trailersData) {
            ...
            for (let item of this.trailersData) {
            ...
            if (this.trailers.length < 1) {
                this.trailers.push({ label: "-", value: undefined });
            }

            this.transportData.TrailerId = this.trailers[0].value;
            this.transportDataForm.patchValue({ TrailerId: this.transportData.TrailerId });
        } else {
    ...
    getCarTypeArray(): void {
        this.carService.getCarType()
            .subscribe(result => {
                this.carTypesData = result.filter(item => item.Type == 2);
